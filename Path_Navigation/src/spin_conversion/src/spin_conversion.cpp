 ...
 */
 ...
 #include "spin_conversion/spin_conversion.hpp"
 ...
 : Node("spin_conversion", options)
 {
   ...
   // 创建SpinCommand订阅器
   spin_command_sub_ = this->create_subscription<spin_conversion::msg::SpinCommand>(
     "spin_command", 1,
     std::bind(&FakeVelTransform::spinCommandCallback, this, std::placeholders::_1));
 }
  ...
  * @param msg 自旋命令消息
  */
 void FakeVelTransform::spinCommandCallback(const spin_conversion::msg::SpinCommand::SharedPtr msg)
 {
   spin_enabled_ = msg->enable_spin;
   spin_direction_ = msg->direction;
   spin_speed_ = spin_enabled_ ? (msg->speed * spin_direction_) : 0.0;
 }
 
 }  // namespace spin_conversion
 
 #include "rclcpp_components/register_node_macro.hpp"
 RCLCPP_COMPONENTS_REGISTER_NODE(spin_conversion::FakeVelTransform)
